...
**レート列の表記について**: 本プロジェクトでは、レート列 $v=(v_1, v_2, v_3, \ldots, v_r)$ は常に**降順（大きいレートから順）**にソートされます。つまり、$v_1$ が最も高いレートを持つアカウントのレート、$v_2$ が2番目に高いレート、といった具合になります。
...
このプロジェクトでは、Python依存関係管理に[uv](https://github.com/astral-sh/uv)を使用しています。
...
```bash
# DPの計算
uv run python -m src.cli dp --n 20 --accounts 3

# 実験の実行
uv run python -m src.experiments.experiment_runner
...
* **レートは降順（高→低）で記録**されます。つまり、account1 が最高レート、account2 が2番目、account3 が最低レートを表します。
...
uv run python -m src.cli dp --n 20 --accounts 3 --initial 1500 1516 1484
```
...
## 8. MCPサーバー連携 (MCP Server Integration)
...
### 9.1 概要
`src/experiments/` 配下に、多アカウント戦略の最適化パターンを可視化するx-Pプロット実験システムを実装しています。このシステムでは、パラメータ（残り試合数n、ベースラインレートv₀、レート差dv等）を変化させたときの各アカウントの期待値変化を分析できます。
...
- `x`: 変更パラメータ（n, v₀, dv等）
- `v1`: 現在の最大レート
- `P1～Pr`: 各アカウント選択時の期待レート
- `Pmax`: 最適行動の期待値（P1～Prの最大値）

#### グラフ仕様
- **縦軸**: P1-Pmax, P2-Pmax, P3-Pmax, ... （各アカウントの期待値と最適行動の差分）
- **横軸**: 変更パラメータ
- **オプション**: 打ち切り基準線（v1-Pmax）の表示
- **解釈**:
  - 0: そのアカウントが最適選択
  - 負の値: 最適行動からのロス量を表示
  - 打ち切り基準線が正: 打ち切りが有利、負: 続行が有利
...
# n-Pプロット実験（基本）
runner.run_n_p_experiment(
    n_values=list(range(1, 21)),
    v_rates=[1500, 1600, 1700],
    experiment_name="basic_n_experiment"
)
...
runner.run_n_p_experiment(
    n_values=list(range(1, 21)),
    v_rates=[1500, 1600, 1700],
    experiment_name="n_experiment_with_cutoff",
    show_cutoff_line=True
)
```

実験結果は `data/experiments/` 配下にCSV、グラフ画像、設定JSONファイルとして保存されます。
