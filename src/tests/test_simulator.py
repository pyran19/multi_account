...
def test_policy_selection():
    ...
    # テスト用の状態を作成
    state = State.from_iterable([1500, 1400, 1300])
...
def test_simulator_single_episode():
    ...
    max_matches = 5
    policy = FixedPolicy(account_idx=0)
...
def test_compare_policies():
    ...
    # ポリシー比較を実行
    results = compare_policies(policies, state, max_matches, episodes)
